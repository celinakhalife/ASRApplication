import { isFunction, flatten, last, head, filter, map } from "lodash";
...
class ASRClient {
  ...
  start(spottingPhrases, callback) {
    ...
    const { languageCode, lingwareTopic, anonymizationEnabled } = config;
  ...
  _handleMessage(event) {
    const msg = this._proto.RecognizeResponse.decode(
  ...
  _processTextResult(result) {
    if (result.final && result.final.alternatives) {
      if (result.final.alternatives[0].normalizedTranscript.length > 0) {
        const phrases = filter(result.phraseSpottingResult, {
          tag: "whitelist"
        });
